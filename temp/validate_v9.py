"""v9: Fix fold_ref to handle Spanish book names in book-specific search.

Key fixes over v8:
1. In book-specific search: use chapter-only extraction (no book name validation needed)
2. Add all Spanish book names to BOOK_FOLD for global fallback
3. Fix short name filter (Ai=2 chars was being dropped)
4. Add more candidate forms (Ephraim→Efraín, Ai→Hai, etc.)
"""
...
BOOK_FOLD = {
    ...
    "rev": "apocalipsis",
    # Spanish book names (bidirectional)
    "josue": "josue",
    "jueces": "jueces",
    "hechos": "hechos",
    "mateo": "mateo",
...
def gen_candidates(name):
    ...
    if len(base) > 2 and base[-1] == base[-2]:
        cands.add(base[:-1])
    # Special known Spanish forms that don't follow phonetic rules
    if base.lower() == "ephraim":
        cands.add("Efraín")
    if base.lower() == "ai":
        cands.update(["Hai", "Haí", "Ha"])
    return sorted([c for c in cands if len(c) >= 2], key=len, reverse=True)


def extract_chapter(ref):
    """Extract just the chapter number from a reference string (no book validation)."""
    if not ref:
        return None
...
def fold_ref(ref):
    ...
    Now handles all Spanish book names."""
    if not ref:
        return None
    ref = ref.strip().replace("\u2013", "-").replace("\u2014", "-")
    m = re.match(
...
def process(loc):
    ...
    # Search English to get book + chapter (try up to 5 results, validate name in text)
    ...
    hit = None
    for h in r["results"]:
        if text_contains_form(h["text"], base):
            hit = h
            break
    if not hit:
        result["note"] = "no_en_passage"
    ...
    # Phase 1: Book-specific search (no book name validation needed)
    for cand in cands:
    ...
    # Phase 2: Global fallback (uses full fold_ref with Spanish book names)
    for cand in cands:
        rs = mc(
            "search_text",
