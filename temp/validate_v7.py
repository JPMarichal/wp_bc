"""v7: English→Spanish location translation via cross-reference matching.

Strategy:
1. Search English for name → get reference + file_path + book_folder + chapter
2. Generate Spanish candidate forms (Rama→Ramá, Gibeah→Gabaa, etc.)
3. Search each candidate in ALL Spanish scriptures (no source_filter)
4. For each result, extract its reference (e.g. "Jueces 20:14-19")
5. If the Spanish reference matches the SAME book+chapter as English → CORRECT match
6. Extract the exact Spanish form from the text
"""
...
# Book name folding for reference comparison
BOOK_FOLD = {
    "1 samuel": "1-samuel",
    "2 samuel": "2-samuel",
    "1 kings": "1-kings",
...
def gen_candidates(name):
    """Generate Spanish candidate forms from English name."""
...
def fold_ref(ref):
    """Extract normalized (book_slug, chapter) from a reference string."""
    if not ref:
        return None
...
def text_contains_form(text, cand):
    """Check if cand (or variant) appears as a proper noun in text."""
...
def process(loc):
    ...
    # 2. Search English to get reference + file_path
    ...
    # Get English (book_slug, chapter) from file_path
    ...
    # 4. Search each candidate in ALL Spanish scriptures
    found_best = None
    found_ref = None
    ...
    for cand in cands:
        ...
        for hit2 in rs["results"]:
            ...
            text = hit2["text"]
            fpath_es = hit2.get("file_path", "")

            # Check if the form appears as proper noun
            match = text_contains_form(text, cand)
            if not match:
                continue

            # Verify same book+chapter
            folded = fold_ref(ref_es)
            if not folded:
                continue
            es_book, es_ch = folded

            # Match: same book slug (either exact or via fold), same chapter
            # For books where ES folder name might differ, try both
            if es_ch == ch_en:
                # Check if books match (es_book might be EN name or ES name)
                es_folder = es_book  # e.g. "jueces" or "1-samuel"
                en_folder = book_en_slug  # e.g. "judges" or "1-samuel"

                # Same folder name in both languages
                if es_folder == en_folder:
                    found_best = match
                    found_ref = ref_es
                    break
                # Different but known mapping
                BOOK_ES_FOLDER = {
                    "jueces": "judges",
                    "josue": "joshua",
                    "exodo": "exodus",
                ...
                if (
                    es_folder in BOOK_ES_FOLDER
                    and BOOK_ES_FOLDER[es_folder] == en_folder
                ):
                    found_best = match
                    found_ref = ref_es
                    break
            elif es_book == book_en_slug:
                # Same book slug (e.g. "1-samuel" for both)
                if es_ch == ch_en:
                    found_best = match
                    found_ref = ref_es
                    break
    ...
    # 5. Fallback: accept any result where chapter matches
    if not found_best:
        for cand in cands:
            rs = mc(
                "search_text",
                {"query": cand, "limit": 5, "source_filter": "es/scriptures"},
            )
            time.sleep(0.1)
            if not rs or not rs.get("results"):
                continue
    ...
    if found_best:
        ...
        result["note"] = f"xref|{found_ref}" if found_ref else "xref"
    else:
...
def main():
    ...
    for idx, loc in enumerate(to_process):
        ...
        if stats["total"] % 20 == 0:
            ...
            sys.stderr.write(
                f"--- [{stats['total']}/{len(to_process)}] F:{stats['found']}({pct}%) HM:{stats['hard_map']} NF:{stats['not_found']} E:{stats['errors']} ---\n"
            )
